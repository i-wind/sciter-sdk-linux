...
value DB_last(VM *vm)
{
  ...
  {
    throw_error(vm, u"DB is already closed");
    return v_undefined();
  }
}
...
bool bind_params(VM *vm, sqlite3_stmt *pst, int argn = 3) // 0-this,1-super,2-sql,3-...
{
  ...
  for (; argn < argv.length(); ++argn)
  {
    ...
    {
      throw_error(vm, u"wrong type of SQL parameter value");
      return false;
    }
  }
  return true;
...
value DB_exec(VM *vm)
{
  std::ustring src; //statement, string with optional '?'s
  pinned db_obj;
  try
  {
  ...
  sqlite3_stmt *pst = 0;
  const char16_t *tail = src.c_str();
  int length = sizeof(char16_t) * src.length();
  ...
  do
  {
    ...
    if (r != SQLITE_OK)
    {
      throw_error(vm, (const char16_t *)sqlite3_errmsg16(pdb));
      return v_null();
    }
    ...
    if (tail)
      // length = wcslen((char16_t *)tail) * sizeof(char16_t);
      length = std::char_traits<char16_t>::length((char16_t *)tail);
    else
      length = 0;
