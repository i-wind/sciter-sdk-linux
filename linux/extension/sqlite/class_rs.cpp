...
{
  throw_error(vm, u"Recordset is creatable only by DB.exec()");
  return v_null();
}
...
static value RS_next(VM *vm)
{
  ...
  if (!pst)
  {
    throw_error(vm, u"Recordset is already closed");
    return v_bool(false);
  }
...
static value RS_name(VM *vm)
{
  ...
  try
  {
    args(vm) >> rs_obj // 'this' is the RS here, instance method.
        ...
        >> colno >> args::optional >> which;
  }
  catch (args::error &e)
  {
    throw_error(vm, aux::a2w(e.msg()));
    return v_undefined();
  }
  ...
  const char16_t *name = 0;
  if (which == 0 || which == v_symbol("logical"))
    name = (const char16_t *)sqlite3_column_name16(pst, colno);
  else if (which == v_symbol("field"))
    name = (const char16_t *)sqlite3_column_origin_name16(pst, colno);
  else if (which == v_symbol("table"))
    name = (const char16_t *)sqlite3_column_table_name16(pst, colno);
  else if (which == v_symbol("database"))
    name = (const char16_t *)sqlite3_column_database_name16(pst, colno);
  else
  {
    throw_error(vm, u"Unknown type of name");
    return v_bool(false);
  }
  if (name)
...
value field_to_value(VM *vm, sqlite3_stmt *pst, int n)
{
  ...
  if (n >= 0 && n < total)
    ...
    {
    case SQLITE_INTEGER:
      return v_int(sqlite3_column_int(pst, n));
    case SQLITE_FLOAT:
      return v_float(sqlite3_column_double(pst, n));
    case SQLITE_TEXT:
      return v_string(vm, (const char16_t *)sqlite3_column_text16(pst, n), sqlite3_column_bytes16(pst, n) / sizeof(char16_t));
    case SQLITE_BLOB:
      return v_bytes(vm, (const unsigned char *)sqlite3_column_blob(pst, n), sqlite3_column_bytes(pst, n));
    case SQLITE_NULL:
      return v_null();
    default:
      throw_error(vm, u"Unknown type of field");
    }
...
static value get_item(VM *vm, value rs_obj, value key)
{
  ...
  {
    std::ustring name = c_symbol(key);
    for (n = 0; n < total; ++n)
    {
      const char16_t *cn = (const char16_t *)sqlite3_column_name16(pst, n);
      if (!cn)
        break;
      if (name == cn)
  ...
  throw_error(vm, u"Field with the index is not found");
  return v_undefined();
}
